function IController(;qmin = 0.2,qmax = 10.0,qsteady_min = 1.0,qsteady_max = 1.0,gamma = 0.9, qold = 1e-4)
    ...
    return IController(qmin,qmax,qsteady_min,qsteady_max,gamma,qold)
end

function update_EEst!(cache::PseudoTransientCache)
    @unpack u,u_prev,u_prev2, alpha,alpha_prev,atmp,tmp = cache
    ...
    @.. broadcast=false tmp =r * cache.internalnorm((u - u_prev) / alpha1 -
                                                             (u_prev - u_prev2) / alpha2)
    #@info "tmp = $(tmp) in eest"

    #=if cache.stats.nsteps % 10 == 0
        push!(alpha_contain,alpha)
    end=#
    calculate_residuals!(atmp,tmp,u_prev,u,cache.abstol,cache.abstol,(u,t) -> cache.internalnorm(u),1.0)
